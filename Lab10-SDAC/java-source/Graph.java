...
public class Graph<V> {
    ...
    private void dfsDAG(Node<V> node){
        node.state = Node.Status.EXPLORING;

        for(Node<V> n : this.getNodes()){
            if(n.state == Node.Status.UNEXPLORED) dfsDAG(n);
            else if(n.state == Node.Status.EXPLORING)
    ...
    public Graph<V> transpose(){
        Graph<V> g = new Graph<>();

        for(Node<V> u : this.getNodes()){
            for(Node<V> v : u.outEdges){
                Node<V> n1 = g.addNode(u.value);
                Node<V> n2 = g.addNode(v.value);
                g.addEdge(n2, n1);
            }
        }
